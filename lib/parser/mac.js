...
MacParser.prototype._processFooter = function (line) {
    ...
    var m;
    if(m = line.match(/(\d+)\%/)){
        this._response.loss = parseInt(m[1]);
    }
    if(m = line.match(/(\d+) packets/)){
        this._response.sent = parseInt(m[1]);
    }
    if(m = line.match(/(\d+) received/)){
        this._response.received = parseInt(m[1]);
    }
    var count = (line.match(/[\/]/g) || []).length;
    if (count >= 3) {
        var regExp = /([0-9\.]+)/g;
